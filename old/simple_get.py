...
def get_pr_list(pr_data, pr_stats, token):
    for pr in pr_data:
...
def get_cm_list(pr_data, cm_stats, token):
    for pr in pr_data:
        comments = get_url(pr["_links"]["comments"]["href"] + "?state=all", token)
        comments.append(get_url(pr["_links"]["review_comments"]["href"] + "?state=all", token))
...
def get_stats(token, repos, filename, timeframe, style):
    ...
    for repo in repos:
        aurl = repo + "/pulls"
        if style == 1:
            get_pr_list(get_prs(aurl, timeframe, token), output, token)
        elif style == 2:
...
def prog_entry():
    ...
    get_stats(args.token, args.repos.split(','), args.output_file, args.timeframe, int(args.style))
